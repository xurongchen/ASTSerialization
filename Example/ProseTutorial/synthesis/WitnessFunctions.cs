...
{
    public class Regex2 : ASTSerialization.IObjSerializable
    {
        public Regex first,second;
        public Regex Item1 => first;
        public Regex Item2 => second;
        public Regex2(Regex _first,Regex _second)
        {
            first = _first;
            second = _second;
        }
        public static Regex2 Create(Regex first,Regex second)
        {
            return new Regex2(first,second);
        }

        public Type getSerializedType()
        {
            return typeof(Regex2);
        }

        public XElement serialize()
        {
            var xe= new XElement("Regex2");
            xe.SetAttributeValue("Item1",Item1.ToString());
            xe.SetAttributeValue("Item2",Item2.ToString());
            return xe;
        }
        public Regex2(XElement xe)
        {
            first = new Regex(xe.Attribute("Item1").Value);
            second = new Regex(xe.Attribute("Item2").Value); 
        }
    }
